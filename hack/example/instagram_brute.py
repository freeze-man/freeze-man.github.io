            ...
            if e.getcode() == 400 or e.getcode() == 403:
                if e.read().decode("utf8", 'ignore').find('"checkpoint_required"') != -1:
                    ...
                    print(bcolors.WARNING +
                          "[!]Error: Proxy IP %s is now on Instagram ,  Removing from working list !" % (proxy,)
                          + bcolors.ENDC
                          )
                    ...
                    print(bcolors.FAIL + "[!]Error : Your Ip is now on Instagram ,"
